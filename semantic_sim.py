...
def clean(text):
        text = text.split(" ")
        #text = [words for words in word_tokenize(text) if word not in punctuations]
        new_words = [word for word in text if word not in stop]
        return text
...
def intro():
    ...
    if option == 1:
        ...
        print(prob_sim_sent)
        '''
        result1 = script.mainTextGet(sent_one, sent_two)
        prob_sim_sent = main(sent_one , sent_two)

        if prob_sim_sent>result1:
                finalAnswer = prob_sim_sent
        else:
                finalAnswer = result1
        print(finalAnswer)
       '''
